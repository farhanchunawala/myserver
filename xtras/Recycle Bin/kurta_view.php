...
<?php
	...
	$query3 = "SELECT * FROM style WHERE sr=$sr AND garb_type=\"kurta\" ORDER BY style_id ASC";
	
	$result3 = mysqli_query($dbc, $query3)
		or die('Error querying database query3.');
	
	
	$query4 = "SELECT * FROM entry WHERE sr=$sr AND garb_type=\"kurta\" ORDER BY garb_id ASC";
	...
	$result4 = mysqli_query($dbc, $query4)
		...
		while ($rowq4 = mysqli_fetch_array($result4)) {
			...
			$kurta_pana[$i] = $rowq4['garb_pana'];
			$kurta_clothln[$i] = $rowq4['garb_clothln'];
		...
		while ($rowq3 = mysqli_fetch_array($result3)) {
			...
			$kurta_collartype[$i] = $rowq3['kurta_collartype'];
			$kurta_cuffln[$i] = $rowq3['kurta_cuffln'];
			$kurta_cufftype[$i] = $rowq3['kurta_cufftype'];
			$kurta_pockettype[$i] = $rowq3['kurta_pockettype'];
			$kurta_shouldertype[$i] = $rowq3['kurta_shouldertype'];
	...
	{
		$kurta_count = $rowq2['kurta_count'];
		$kurta_type = $rowq2['kurta_type'];
		$kurta_length = $rowq2['kurta_length'];
		$kurta_shoulder = $rowq2['kurta_shoulder'];
		$kurta_sleeve = $rowq2['kurta_sleeve'];
		$kurta_chest = $rowq2['kurta_chest'];
		$kurta_stomach = $rowq2['kurta_stomach'];
		$kurta_seat = $rowq2['kurta_seat'];
		$kurta_sleevefit = $rowq2['kurta_sleevefit'];
		$kurta_biceps = $rowq2['kurta_biceps'];
		$kurta_collar = $rowq2['kurta_collar'];
		$kurta_cuffbr = $rowq2['kurta_cuffbr'];
		$kurta_pocketdown = $rowq2['kurta_pocketdown'];
		$kurta_pocket = $rowq2['kurta_pocket'];
		$kurta_hala = $rowq2['kurta_hala'];
		$kurta_t1 = $rowq2['kurta_t1'];
		$kurta_t2 = $rowq2['kurta_t2'];
		$kurta_t3 = $rowq2['kurta_t3'];
	}
	...
	$x_front_stomach2 = $kurta_shoulder;
	...
	{	// sleeve
	$x_sleeve_length1 = $kurta_sleeve - ($kurta_cuffln[$i] - 1) + 1;
	$x_sleeve_length2 = $kurta_sleeve - ($kurta_cuffln[$i] - 1) + 1 + 0.5;
	if ($kurta_cuffbr==9) {
		$y_sleeve_cuff = 5.5;
	} elseif ($kurta_cuffbr==9.25) {
		$y_sleeve_cuff = 5.625;
	} elseif ($kurta_cuffbr==9.5) {
		$y_sleeve_cuff = 5.75;
	} elseif ($kurta_cuffbr==9.75) {
		$y_sleeve_cuff = 5.875;
	} elseif ($kurta_cuffbr==10) {
		$y_sleeve_cuff = 6;
	} elseif ($kurta_cuffbr==10.25) {
		$y_sleeve_cuff = 6.125;
	} elseif ($kurta_cuffbr==10.5) {
		$y_sleeve_cuff = 6.25;
	} else {
		$y_sleeve_cuff = $kurta_cuffbr / 2 + 0.5;
	}
